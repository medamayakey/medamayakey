...
import { deleteShoppingItem, getShoppingListItems } from "@/actions/db/firebase/firestore";
...
export default function Cartbar() {
  ...
  const getTotalAmount = () => {
    const totalAmount = cartItems.reduce((amount, cartItem) => amount + cartItem.price, 0);
    return totalAmount.toFixed(2);
  };

  return (
    <aside className="max-w-72 border-l">
          ...
          <ScrollArea className="h-80">
            <ul>
              {cartItems.map((cartItem: Item) => (
                <li value={cartItem.id} key={cartItem.id} id={cartItem.id} className="border-b px-6 p-2">
                  <div className="flex justify-between items-center w-full gap-4">
                    <div>
                      <h3 className="mb-1">{cartItem.name}</h3>
                      <p className="text-slate-700 text-sm">${cartItem.price}</p>
                    </div>
                    <button className="w-8" onClick={() => handleDeleteItem(cartItem.id as string)}>
